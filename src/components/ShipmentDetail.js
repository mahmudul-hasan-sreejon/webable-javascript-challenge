...
class ShipmentDetail extends Component {
    ...
    componentDidMount() {
        axios
        .get(`http://localhost:3001/shipments?id=${this.state.id}`)
        .then((response) => {
    ...
    render() {
        const details = this.state.details;
        if(!details) return <h1 className="loading-error">Loading...</h1>;
        ...
        return (
            <div className="shipment-datail-container">
                ...
                <p className="shipment-detail-label">Cargo :</p>
                {
                    details.cargo.map((el, index) => {
                        return (
                            <ul key={index}>
                                ...
                                <li>Volume: {el.volume}m<sup>3</sup></li>
                            </ul>
                        );
                    })
                }

                <p className="shipment-detail-label">Services :</p>
                {
                ...
                <Modal 
                    modelStatus={this.state.isModalOpen}
